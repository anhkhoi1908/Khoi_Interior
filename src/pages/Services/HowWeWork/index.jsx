...
const HowWeWork = () => {
    return (
        <Fragment>
            <div id="how_we_work" className="w-3/5 mx-auto bg-[#F4F0EC] rounded-3xl pt-[8rem] pb-[6rem]">
                ...
                <ul className="mt-20 [&>*:nth-child(even)]:flex-row-reverse pl-0">
                    {how_we_do.map((el, index) => (
                        <li key={index} className="flex items-center justify-evenly mb-20">
                            <img src={`/assets/imgs/${el.image}`} width={400} height={300} className="img_main"/>
                            <div className="w-2/5">
                                <img src={`/assets/imgs/${el.image_sub}`} className="img_sub"/>
